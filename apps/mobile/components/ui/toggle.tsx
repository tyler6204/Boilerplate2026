...
function Toggle({
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        'font-footnote text-foreground font-medium',
        props.pressed
...
function ToggleIcon({ className, ...props }: React.ComponentProps<typeof IconSymbol>) {
  const textClass = React.useContext(TextClassContext);
  return <IconSymbol className={cn('size-4 shrink-0', textClass, className)} {...props} />;
}
